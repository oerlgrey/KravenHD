# -*- coding: utf-8 -*-
#
#  KravenHDOAWeather Converter
#
#  Modified/Adapted by oerlgrey
...
class KravenHDOAWeather(Converter, object):
	CURRENT = 0
	DAY1 = 1
	DAY2 = 2
	DAY3 = 3
	...
	def getText(self):
		try:
			if self.mode == self.WEATHERSOURCE:
				...
				return self.source.getTemperature().replace(" ","")
			elif self.mode == self.FEELSLIKE:
				return self.source.getTemperature() + _(", feels ") + self.source.getFeeltemp()
			elif self.mode == self.HUMIDITY:
				return self.source.getHumidity().replace("% %", _("% humidity"))
				...
				return self.source.getMaxTemp(self.index).replace(" ","")
			elif self.mode == self.TEMPERATURE_LOW and self.index is not None:
				return self.source.getMinTemp(self.index).replace(" ","")
			elif self.mode == self.TEMPERATURE_HIGH_LOW and self.index is not None:
				return self.source.getMaxMinTemp(self.index).replace(" °C - ","° / ").replace(" °C", "°").replace(" °F - ","° / ").replace(" °F", "°")
